...
class EnforcementBrowserTest extends EnforcementBrowserTestBase {
  ...
  public function testOutOfStock() {
    $this->stockServiceManager->getService($this->variation)
      ->getStockUpdater()
      ->createTransaction($this->variation, $this->locations[1]->getId(), '', -10, StockTransactionsInterface::STOCK_OUT, $this->adminUser->id(), 1234, NULL, 12.12, 'USD');
    $this->drupalGet('product/' . $this->product->id());
  ...
  protected function createProduct() {
    ...
    $this->stockServiceManager->getService($this->variation)
      ->getStockUpdater()
      ->createTransaction($this->variation, $this->locations[1]->getId(), '', 1, StockTransactionsInterface::STOCK_IN, $this->adminUser->id(), 1234, NULL, 12.12, 'USD', []);
  }
