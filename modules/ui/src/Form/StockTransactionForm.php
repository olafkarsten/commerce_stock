...
class StockTransactionForm extends FormBase {
   ...
   */
  protected function getTransactionTypeForms() {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $stockServiceConfiguration = $stockService->getConfiguration();

    $context = $this->getContext($productVariation);
    $locations = $stockServiceConfiguration->getAvailabilityLocations($context, $productVariation);
    $location_options = [];
    /** @var \Drupal\commerce_stock\StockLocationInterface $location */
    foreach ($locations as $location) {
      $location_options[$location->getId()] = $location->getName();
    }
    $transactionTypeForms = $this->getTransactionTypeForms();
    $transactionOptions = $this->getOptionLabels($transactionTypeForms);
